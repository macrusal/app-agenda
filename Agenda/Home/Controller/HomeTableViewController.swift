...
class HomeTableViewController: UITableViewController, UISearchBarDelegate, NSFetchedResultsControllerDelegate {
    ...
    @objc func abrirActionSheet(_ longPress:UILongPressGestureRecognizer) {
        if longPress.state == .began {
            guard let alunoSelecionado = gerenciadorDeResultados?.fetchedObjects?[(longPress.view?.tag)!]
                else {return}
            let menu = MenuOpcoesAlunos().configuraMenuOpcoesAluno(completion: {(opcao) in
                ...
                case .waze:
                    ...
                    if UIApplication.shared.canOpenURL(URL(string: "waze://")!) {
                        Localizacao().converterEnderecoEmCoordenadas(enderecoDoAluno, local: { (localizacaoEncontrada) in
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let contadorDaListaDeAlunos = gerenciadorDeResultados?.fetchedObjects?.count else
        {return 0}
            ...
            AutenticacaoLocal().autorizaUsuario { (autenticado) in
                if autenticado {
                    DispatchQueue.main.async {
                        guard let alunoSelecionado = self.gerenciadorDeResultados?.fetchedObjects![indexPath.row]
                            else {return};
    ...
    @IBAction func buttonCalcularMedia(_ sender: UIBarButtonItem) {
        ...
        CalculaMediaAPI().calcularMediaGeralDosAlunos(alunos: listaDeAlunos, sucesso: { (dicionario) in
            if let alerta = Notificacoes().exibiNotificacaoDeMediaDosAlunos(dicionarioDeMedia: dicionario) {
